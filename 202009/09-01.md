...
如果放在底部，页面结构可能在加载css的时候会发生变化，如果放在头部，render tree会结合cssom一起渲染
...
让加载更快：
- 减少资源体积：压缩代码
- 减少访问次数：合并代码，SSR服务端渲染，缓存、css sprites
- 使用更快的网络：CDN
...
场景：拖拽一个元素，要随时拿到该元素被拖拽的位置，可以减少触发次数，每隔100ms再触发
```js
...
- 在json普及之前，ajax实际上是用xml作为上传格式
- json文件的key和value都必须用双引号括起来，与js中的json对象不一样
